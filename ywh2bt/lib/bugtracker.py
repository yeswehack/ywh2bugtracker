...
class BugTracker:
    ...
    ywh_comment_marker = "Imported to the bugtracker."
...
"""
    ...
    def file_to_base64(self, img: bytes, content_type=None) -> str:
        ...
        f = io.BytesIO(img)
        mime = imghdr.what(f)
        if mime is None and (not content_type or "image/" not in content_type or "svg" in content_type):
            raise ValueError("Image type not supported")
        mime = mimetypes.types_map["." + mime] if mime else content_type
        image = "data:" + mime + ";base64," + str(base64.b64encode(img).decode("utf-8"))
        return image
